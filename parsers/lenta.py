...
# Задержка между запросами к статьям, чтобы не получить бан
ARTICLE_DELAY = 0.4  # секунд


def parse_lenta() -> list[dict]:
    """
    Парсит последние новости с главной страницы Lenta.ru.
    ...
    for url, hint_title in article_links:
        ...
        try:
            ...
            # Дедупликация на уровне парсера
...
def _collect_article_links(soup: BeautifulSoup) -> list[tuple[str, str]]:
    """
    Возвращает список (article_url, hint_title) с главной страницы.
    Приоритет — сегодняшние статьи по формату URL /YYYY/MM/DD/slug/.
    """
    ...
    for tag in soup.find_all("a", href=True):
        ...
        # Фильтруем только статьи Lenta: /YYYY/MM/DD/slug/
        ...
        # Не берём ссылки на разделы (года слишком малы или велики)
        ...
        # Пробуем извлечь подсказку по заголовку прямо из тега
        hint = _hint_title(tag)

        article_date = f"{year:04d}-{month:02d}-{day:02d}"
        if article_date == today:
            today_links.append((full_url, hint))
        else:
            other_links.append((full_url, hint))

    # Сначала сегодняшние, потом остальные
    result = today_links + other_links
...
def _hint_title(tag) -> str:
    """Пытается извлечь заголовок прямо из тега-ссылки (без запроса к статье)."""
...
def _fetch_article(url: str, hint_title: str) -> dict | None:
    ...
    try:
        ...
        # Если не смогли загрузить статью — используем подсказку без картинки
    ...
    # ── Картинка (og:image всегда уникальна для каждой статьи) ────────────────
    image_url = _meta_content(soup, "og:image") or ""

    # Запасной вариант: первая крупная картинка в статье
    if not image_url:
        for selector in [
            "div.topic-body img",
